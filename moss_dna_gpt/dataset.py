...
def windows(seq:str,block_size:int=1024,stride:int=512,max_n_rate:float=1.0,invalid_policy:str='skip'):
    if block_size<=0 or stride<=0: raise ValueError('block_size and stride must be positive')
    seq=seq.upper(); out=[]; stats={'candidates':0,'dropped_n_rate':0,'dropped_invalid':0,'dropped_short':0}
...
def prepare_windows_from_fasta(fasta,out_dir,block_size=1024,stride=512,max_n_rate=1.0,train_ratio=0.8,val_ratio=0.1,seed=42,shuffle=True,invalid_policy='skip'):
    allw=[]; per=[]
    for r in iter_fasta(fasta):
        ws,st=windows(r.sequence,block_size,stride,max_n_rate,invalid_policy); allw+=ws; per.append({'name':r.name,'length':r.length,'kept':len(ws),**st})
    ...
    manifest={'fasta':str(fasta),'block_size':block_size,'stride':stride,'max_n_rate':max_n_rate,'total_windows':len(allw),'splits':{k:len(v) for k,v in parts.items()},'sequences':per}
